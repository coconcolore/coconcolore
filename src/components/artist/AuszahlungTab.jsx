...
const statusConfig = {
  beantragt: { label: 'Beantragt', color: 'bg-amber-100 text-amber-700', icon: Clock },
  verarbeitet: { label: 'Überwiesen', color: 'bg-primary/10 text-primary', icon: CheckCircle },
...
export default function AuszahlungTab({ existingProfile, user }) {
  ...
  // Ausstehende Anträge ebenfalls abziehen, um Doppel-Auszahlungen zu verhindern
  ...
  const payoutMutation = useMutation({
    mutationFn: async (data) => {
      // Erst PayoutRequest anlegen
      const payoutRequest = await api.entities.PayoutRequest.create(data);
      // Dann direkt als verarbeitet über Stripe-Backend ausführen
    ...
    onSuccess: () => {
      ...
      setAmount('');
      setNotes('');
      toast.success('Auszahlung wurde direkt initiiert!');
    },
    onError: (err) => {
      queryClient.invalidateQueries({ queryKey: ['my-payouts', user?.email] });
      toast.error('Fehler bei der Auszahlung: ' + (err?.message || 'Unbekannter Fehler'));
    },
  });
  ...
  const handleSubmit = (e) => {
    ...
    if (amt > available) return toast.error('Betrag übersteigt verfügbares Guthaben.');
    if (!iban.trim()) return toast.error('Bitte IBAN eingeben.');
    if (hasPendingPayout) return toast.error('Es gibt bereits eine ausstehende Auszahlung.');
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-3">
        <Card className="p-4 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">Gesamteinnahmen</p>
        </Card>
        <Card className="p-4 text-center">
          <p className="text-xl font-bold font-display">{totalPaidOut.toFixed(2)} €</p>
          <p className="text-xs text-muted-foreground mt-1">Ausgezahlt</p>
        </Card>
      ...
      <Card className="p-5">
        <h3 className="font-semibold mb-4 flex items-center gap-2">
          <Banknote className="w-4 h-4 text-primary" />Bankverbindung hinterlegen
        </h3>
        <div className="space-y-2">
          ...
          {existingProfile?.iban && <p className="text-xs text-muted-foreground">Gespeichert: {existingProfile.iban}</p>}
        </div>
      </Card>
      ...
      <Card className="p-5 border-amber-200 bg-amber-50">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="font-semibold text-amber-800">Auszahlungen noch nicht verfügbar</p>
            <p className="text-sm text-amber-700 mt-1">
              Zur Auszahlung gibt es in den nächsten Tagen noch weitere Infos 
            </p>
          </div>
        </div>
      </Card>
