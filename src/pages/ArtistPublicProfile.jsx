...
export default function ArtistPublicProfile() {
  ...
  if (!artist) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4">
      <h2 className="font-display text-2xl font-bold">Profil nicht gefunden</h2>
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-8 py-12">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="font-display text-2xl font-semibold">Kurse von {artist.display_name}</h2>
          <Badge variant="secondary">{courses.length}</Badge>
        </div>
        {courses.length === 0 ? (
          <p className="text-muted-foreground py-8 text-center">Noch keine Kurse veröffentlicht</p>
        ) : (
